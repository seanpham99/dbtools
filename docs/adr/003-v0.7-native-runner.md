...
Decision 12 turned out to be mostly unnecessary. v0.7 reads the same
`dbtools_migration_history` table v0.6 wrote and derives the version from
rows v0.6 already maintained, so an upgrade needs no import at all — only a
widened status CHECK constraint, which `EnsureSchema` applies in place.
`adopt` remains the path for databases migrated by *another* tool.
