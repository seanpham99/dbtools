...
func TestBuildRows_MixedSuccessAndErrors(t *testing.T) {
	...
	fakeCollect := func(databaseURL, engineName, migrationsDir, upSuffix, ledgerTable, targetName string) (*statusinfo.Status, error) {
		if targetName == "prod" {
...
func TestBuildRows_EmptyConfig(t *testing.T) {
	...
	rows := BuildRows(cfg, func(string, string, string, string, string, string) (*statusinfo.Status, error) {
